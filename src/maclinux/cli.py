...
def cmd_diagnose(args: argparse.Namespace) -> int:
    result = detect()
    print(f"Mac Linux Hardware Support 0.1.0")
    print(f"Model: {result['model']}")
    print(f"Kernel: {result['kernel']}")
    print(f"Architecture: {result['architecture']}")
...
def cmd_repair(args: argparse.Namespace) -> int:
    ...
    if args.component == "camera":
        print("Camera repair is not automated yet.")
        print("Detected dependency: patjak/facetimehd + facetimehd-firmware.")
        print("Next MVP step: distro adapter + kernel compatibility check + safe build/install plan.")
        return 3
